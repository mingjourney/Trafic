...
const getQueryString = (line: string, start: string, end: string) =>
  `{
      sectionFlow(line: "${line}", dateRange: {start: "${start}", end: "${end}"}) {
        up {
          source
          target
          flow
...
const SectionFlow = ({
  ...
  const renderChart = (sectionFlow: any) => {
    ...
    chart.setOption({
      ...
      xAxis: {
        type: "category",
        data: [
          trimStation(sectionFlow.up[0].source),
          ...sectionFlow.up.map((e: any) => trimStation(e.target)),
        ],
        axisLabel: {
      ...
      series: [
        {
          data: [...sectionFlow.up.map((e: any) => e.flow), 0],
          stack: "total",
          type: "bar",
        ...
        {
          data: [0, ...sectionFlow.down.map((e: any) => -e.flow)],
          stack: "total",
          type: "bar",
          emphasis: emphasisStyle,
          color: new echarts.graphic.LinearGradient(0, 1, 0, 0, [
  ...
  useEffect(() => {
    fetcher(getQueryString(line, ...dateRange)).then((data) => {
      renderChart(data.sectionFlow);
    });
  }, [line, dateRange]);
  useEffect(() => {
    if (!ref.current) return;
